...
object FeetMetersConverters {
  ...
  implicit class DoubleToMeters(d: Double) {
  ...
  /* ----------------------------------- */
  implicit val feetToMeters: Feet => Meters = (f) => Meters(f.length/FEET_IN_ONE_METER)
  implicit val metersToFeet: Meters => Feet = (m) => Feet(m.length * FEET_IN_ONE_METER)
